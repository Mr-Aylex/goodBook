...
class Model:
    ...
    def create_layers(self, inputs1, inputs2, inputs3):
        regularizer = keras.regularizers.l2(0.0008)
        dropout_rate = 0.10
        ...
        x = keras.layers.LocallyConnected1D(32, 5, kernel_regularizer=regularizer, bias_regularizer=regularizer,
                                            padding="valid", activation=keras.activations.relu)(x)
        x = keras.layers.SpatialDropout1D(dropout_rate)(x)

        x = keras.layers.LocallyConnected1D(16, 2, kernel_regularizer=regularizer, bias_regularizer=regularizer,
                                            padding="valid", activation=keras.activations.relu)(x)
        x = keras.layers.SpatialDropout1D(dropout_rate)(x)
