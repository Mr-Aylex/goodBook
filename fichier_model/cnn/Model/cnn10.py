...
class Model:
    ...
    def create_layers(self, inputs1, inputs2, inputs3):
        ...
        x = keras.layers.Embedding(len(self.vocabulary) + 1, 128, batch_size=100,
                                   embeddings_regularizer=keras.regularizers.l2(0.0008))(
            vectorize_layer)
        ...
        x = keras.layers.Conv1D(40, 4, activation='relu', padding='valid', kernel_regularizer=regularizer)(x)
        x = keras.layers.SpatialDropout1D(dropout_rate)(x)

        x = keras.layers.Conv1D(30, 3, activation='relu', padding='valid', kernel_regularizer=regularizer)(x)
        x = keras.layers.SpatialDropout1D(dropout_rate)(x)
