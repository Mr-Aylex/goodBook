...
def double_conv_block(x, n_filters):
    ...
    x = layers.Conv1D(n_filters, 12, padding="same", activation="relu", kernel_initializer="he_normal")(x)
    # Conv2D then ReLU activation
    x = layers.Conv1D(n_filters, 12, padding="same", activation="relu", kernel_initializer="he_normal")(x)
    return x
...
def model(vocabulary, dropout_rate=0.15, kernel_regularizer=regularizers.L1L2(l1=1e-5, l2=1e-4),
    ...
    f1, p1 = downsample_block(x, 32, dropout_rate)
    f2, p2 = downsample_block(p1, 64, dropout_rate)
    f3, p3 = downsample_block(p2, 128, dropout_rate)

    bottleneck = double_conv_block(p3, 256)

    u1 = upsample_block(bottleneck, f3, 128, dropout_rate)

    u2 = upsample_block(u1, f2, 64, dropout_rate)

    u3 = upsample_block(u2, f1, 32, dropout_rate)
    ...
    return keras.Model(inputs=[inputs1, inputs2, inputs3, inputs4, inputs5, inputs6, inputs7], outputs=output, name="unet13")