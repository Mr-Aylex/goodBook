...
def double_conv_block(x, n_filters):
    ...
    x = layers.Conv1D(n_filters, 1, padding="same", activation="relu", kernel_initializer="he_normal")(x)
    # Conv2D then ReLU activation
    x = layers.Conv1D(n_filters, 1, padding="same", activation="relu", kernel_initializer="he_normal")(x)
    return x
...
def model(vocabulary, dropout_rate=0.15, kernel_regularizer=regularizers.L1L2(l1=1e-5, l2=1e-4),
    ...
    return keras.Model(inputs=[inputs1, inputs2, inputs3, inputs4, inputs5, inputs6, inputs7], outputs=output, name="unet11")