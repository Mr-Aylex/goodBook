...
def model(vocabulary, dropout_rate=0.15, kernel_regularizer=regularizers.L1L2(l1=1e-5, l2=1e-4),
    ...
    x = TokenAndPositionEmbedding(len(vocabulary), 352, 220)(vectorize_layer)

    x = TransformerEncoder(180, 6, dropout_rate)(x)
    x = TransformerEncoder(180, 6, dropout_rate)(x)
    x = TransformerEncoder(180, 6, dropout_rate)(x)
    # return only the 6 last element of the sequence
    encoder_out = keras.layers.GlobalAveragePooling1D()(x)
    ...
    dense = keras.layers.Dense(16, activation=keras.activations.relu,
                               ...
                               activity_regularizer=activity_regularizer)(dense)
    dense = keras.layers.Dropout(dropout_rate)(dense)
    output = keras.layers.Dense(6, activation='softmax')(dense)
