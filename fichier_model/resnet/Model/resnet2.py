...
def model(vocabulary, dropout_rate=0.15, kernel_regularizer=regularizers.L1L2(l1=1e-5, l2=1e-4),
    ...
    # Stage 5
    X = convolutional_block(X, f=8, filters=[256, 256, 1024], stage=5, block='a', s=2)
    X = identity_block(X, 8, [256, 256, 1024], stage=5, block='b')
    X = identity_block(X, 8, [256, 256, 1024], stage=5, block='c')
    X = keras.layers.Dropout(dropout_rate)(X)
