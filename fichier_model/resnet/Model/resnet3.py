...
def model(vocabulary, dropout_rate=0.15, kernel_regularizer=regularizers.L1L2(l1=1e-5, l2=1e-4),
    ...
    x = layers.Embedding(len(vocabulary), 220)(vectorize_layer)
    ...
    # Stage 2
    X = convolutional_block(X, f=8, filters=[32, 32, 128], stage=2, block='a', s=1)
    X = identity_block(X, 8, [32, 32, 128], stage=2, block='b')
    X = identity_block(X, 8, [32, 32, 128], stage=2, block='c')
    X = keras.layers.Dropout(dropout_rate)(X)

    # Stage 3
    X = convolutional_block(X, f=8, filters=[64, 64, 256], stage=3, block='a', s=2)
    X = identity_block(X, 8, [64, 64, 256], stage=3, block='b')
    X = identity_block(X, 8, [64, 64, 256], stage=3, block='c')
    X = identity_block(X, 8, [64, 64, 256], stage=3, block='d')
    X = keras.layers.Dropout(dropout_rate)(X)

    # Stage 4
    """X = convolutional_block(X, f=8, filters=[128, 128, 512], stage=4, block='a', s=2)
    X = identity_block(X, 8, [128, 128, 512], stage=4, block='b')
    X = identity_block(X, 8, [128, 128, 512], stage=4, block='c')
    X = identity_block(X, 8, [128, 128, 512], stage=4, block='d')
    X = identity_block(X, 8, [128, 128, 512], stage=4, block='e')
    X = identity_block(X, 8, [128, 128, 512], stage=4, block='f')
    X = keras.layers.Dropout(dropout_rate)(X)"""

    # Stage 5
    X = convolutional_block(X, f=8, filters=[64, 64, 256], stage=5, block='a', s=2)
    X = identity_block(X, 8, [64, 64, 256], stage=4, block='b')
    X = identity_block(X, 8, [64, 64, 256], stage=4, block='c')
    X = keras.layers.Dropout(dropout_rate)(X)
